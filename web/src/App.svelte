<script lang="ts">
  ...
  let cssWidth = $derived(
    Math.max(lcdWidth, (150 * lcdWidth) / lcdHeight, 320),
  );
  ...
  function getPotsAndMultipos(): { input: InputDef; index: number }[] {
    return getFlexInputs().filter(
      ({ input }) =>
        input.default === "POT" ||
        input.default === "POT_CENTER" ||
        input.default === "MULTIPOS",
    );
  }
  ...
  async function discoverRadios() {
    try {
      ...
      if (avail.length > 0) {
        ...
        selectedRadio = match
          ? match.wasm
          : saved && avail.some((r) => r.wasm === saved)
            ? saved
            : avail[0].wasm;
  ...
  async function run() {
    ...
    try {
      ...
      if (canvas) {
        ...
        onTrace(
          `[lcd] ${lcdWidth}×${lcdHeight}@${lcdDepth}bpp css=${Math.round(cssWidth)}×${Math.round(cssHeight)} buf=${canvasW}×${canvasH} dpr=${dpr}\n`,
        );
      }
  ...
  async function resetSdCard() {
    if (!runner) return;
    if (
      !confirm("Wipe all SD card data for this radio? This cannot be undone.")
    )
      return;
    await runner.fsWipe();
    fsHasContent = false;
...
<main>
  <div class="header">
    ...
    <div class="toolbar">
      <select bind:value={selectedRadio} disabled={running || loading}>
      ...
      {#if !running && !loading}
        <button onclick={run} disabled={!selectedRadio}> Run </button>
      {/if}
      {#if running}
        <button onclick={stop}>Stop</button>
      {/if}
      {#if fsReady}
        <button onclick={handleFileUpload} class="secondary"
          >Upload Files</button
        >
        <button onclick={handleFolderUpload} class="secondary"
    ...
    <div class="lcd-bezel">
      {#key canvasKey}
        <canvas
          ...
          style:background={lcdDepth > 0 && lcdDepth < 16
            ? "rgb(47, 123, 227)"
            : "#000"}
  ...
  {#snippet leftGimbalBlock()}
    <div class="gimbal-wrapper gimbal-left">
      {#if (currentRadio?.trims?.length ?? 0) >= 4}
        <div class="trim-col">
          <button
            ...
            ontouchend={() => handleTrimUp(2, "inc")}>+</button
          >
          ...
          <button
            ...
            ontouchend={() => handleTrimUp(2, "dec")}>-</button
          >
        </div>
      {/if}
      <div class="gimbal-center">
        ...
        >
          <div
            class="gimbal-crosshair-h"
            style:top="{gimbalCrossY('left')}%"
          ></div>
          <div
            class="gimbal-crosshair-v"
        ...
        {#if (currentRadio?.trims?.length ?? 0) >= 4}
          <div class="trim-row">
            <button
              ...
              ontouchend={() => handleTrimUp(3, "dec")}>-</button
            >
            ...
            <button
              ...
              ontouchend={() => handleTrimUp(3, "inc")}>+</button
            >
          </div>
        {/if}
      </div>
        ...
        {#if (currentRadio?.trims?.length ?? 0) >= 2}
          <div class="trim-row">
            <button
              ...
              ontouchend={() => handleTrimUp(0, "dec")}>-</button
            >
            ...
            <button
              ...
              ontouchend={() => handleTrimUp(0, "inc")}>+</button
            >
          </div>
        {/if}
      </div>
      {#if (currentRadio?.trims?.length ?? 0) >= 2}
        <div class="trim-col">
          <button
            ...
            ontouchend={() => handleTrimUp(1, "inc")}>+</button
          >
          ...
          <button
            ...
            ontouchend={() => handleTrimUp(1, "dec")}>-</button
          >
        </div>
      {/if}
    </div>
      ...
      <div class="multipos-control">
        ...
        <div class="multipos-buttons">
          {#each [0, 1, 2, 3, 4, 5] as pos}
            <button
              class="multipos-btn"
              class:active={Math.round((analogValues[index] * 5) / 4096) ===
                pos}
              onclick={() => handleMultiposChange(index, pos)}>{pos + 1}</button
            >
          {/each}
        </div>
          ...
          {#each getCustomSwitches() as { sw, index: swIdx }, csIdx}
            <div class="custom-switch">
              <button
                ...
                style:box-shadow={customSwitchColors[csIdx]
                  ? `0 0 8px ${customSwitchColors[csIdx]}`
                  : ""}
                onmousedown={() => updateSwitch(swIdx, 1)}
                onmouseup={() => updateSwitch(swIdx, -1)}
                onmouseleave={() => {
                  if (switchStates[swIdx] === 1) updateSwitch(swIdx, -1);
                }}
                ontouchstart={(e) => {
                  e.preventDefault();
                  updateSwitch(swIdx, 1);
                }}
                ontouchend={(e) => {
                  e.preventDefault();
                  updateSwitch(swIdx, -1);
        ...
        {#each getCustomSwitches() as { sw, index }, csIdx}
          <div class="custom-switch">
            <button
              ...
              onmouseleave={() => {
                if (switchStates[index] === 1) updateSwitch(index, -1);
              }}
              ontouchstart={(e) => {
                e.preventDefault();
        ...
        {#each (currentRadio?.trims ?? []).slice(4) as trim, i}
          <div class="trim-row">
            <button
              ...
              ontouchend={() => handleTrimUp(4 + i, "dec")}>-</button
            >
            ...
            <button
              ...
              ontouchend={() => handleTrimUp(4 + i, "inc")}>+</button
            >
          </div>
        {/each}
      </div>
  ...
  <details>
    <summary class="trace-summary"
      >Trace output{#if running}<span class="fps">{fps} fps</span>{/if}</summary
    >
